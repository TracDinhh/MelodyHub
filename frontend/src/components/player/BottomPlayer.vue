...
<template>
  <footer class="melodyhub-panel-edge fixed inset-x-2 bottom-2 z-[60] h-22 rounded-xl border border-white/[0.08] bg-[#111719]/94 px-3 backdrop-blur-2xl sm:inset-x-3 sm:bottom-3 sm:px-5">
    <div class="grid h-full grid-cols-[minmax(0,1fr)_auto] items-center gap-3 lg:grid-cols-[minmax(210px,1fr)_minmax(360px,1.4fr)_minmax(220px,1fr)]">
      <div class="flex min-w-0 items-center gap-3">
        <img v-if="player.currentTrack.cover" :src="player.currentTrack.cover" :alt="`${player.currentTrack.title} cover`" class="size-13 rounded-lg object-cover ring-1 ring-white/10 sm:size-14" />
        ...
        <button class="melodyhub-icon-btn hidden sm:grid" title="Like track" @click="player.toggleLike(player.currentTrack.id)">
          <Heart :size="17" :class="isLiked ? 'fill-[#1DB954] text-[#1DB954]' : ''" />
        </button>
        <button class="melodyhub-icon-btn hidden md:grid" title="Download for offline" @click="player.toggleDownload(player.currentTrack.id)">
      ...
      <div class="flex flex-col items-center">
        <div class="flex items-center gap-2 sm:gap-4">
          <button class="melodyhub-player-btn hidden sm:grid" title="Shuffle" :class="{ '!text-[#1DB954]': player.shuffle }" @click="player.shuffle = !player.shuffle">
            <Shuffle :size="17" />
          </button>
          <button class="melodyhub-player-btn hidden sm:grid" title="Previous track" @click="player.previous">
            <SkipBack :size="19" class="fill-current" />
          </button>
          <button class="grid size-10 place-items-center rounded-full bg-[#65e78c] text-[#071108] shadow-[0_6px_20px_rgba(29,185,84,0.25)] transition hover:scale-105 active:scale-95 sm:size-11" :title="player.isPlaying ? 'Pause' : 'Play'" @click="player.togglePlayback">
            <Pause v-if="player.isPlaying" :size="20" class="fill-current" />
            <Play v-else :size="20" class="ml-0.5 fill-current" />
          </button>
          <button class="melodyhub-player-btn" title="Next track" @click="player.next">
            <SkipForward :size="19" class="fill-current" />
          </button>
          <button class="melodyhub-player-btn hidden sm:grid" title="Repeat" :class="{ '!text-[#1DB954]': player.repeat }" @click="player.repeat = !player.repeat">
            <Repeat2 :size="17" />
          </button>
        </div>
      ...
      <div class="hidden items-center justify-end gap-2 lg:flex">
        <button class="melodyhub-icon-btn" :title="player.muted ? 'Unmute' : 'Mute'" @click="player.toggleMute">
        ...
        <input
          ...
          max="100"
          :value="player.volume"
          class="melodyhub-range w-24"
          aria-label="Volume"
          :style="{ '--range-progress': `${player.volume}%` }"
          @input="player.setVolume($event.target.value)"
        />
        <button class="melodyhub-icon-btn" title="Open queue" @click="player.queueOpen = true">
          <ListMusic :size="18" />
        </button>
        <button class="melodyhub-icon-btn" title="Fullscreen lyrics" @click="player.fullscreenLyrics = true">
          <Maximize2 :size="17" />
        </button>
      </div>
    </div>
  ...
  <Teleport to="body">
    <div v-if="player.queueOpen" class="fixed inset-0 z-[80] bg-black/60 backdrop-blur-sm" @click.self="player.queueOpen = false">
      <aside class="absolute inset-y-0 right-0 w-[min(420px,92vw)] border-l border-white/10 bg-[#111] p-5 shadow-2xl">
        <header class="flex items-center justify-between">
          ...
          <button class="melodyhub-icon-btn" title="Close queue" @click="player.queueOpen = false"><X :size="18" /></button>
        </header>
    ...
    <div v-if="player.fullscreenLyrics" class="fixed inset-0 z-[90] flex flex-col bg-[#090909]/98 p-6 backdrop-blur-2xl">
      <header class="flex items-center justify-between">
        ...
        <button class="melodyhub-icon-btn" title="Close fullscreen lyrics" @click="player.fullscreenLyrics = false"><X :size="22" /></button>
      </header>
