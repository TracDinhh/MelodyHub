...
<template>
  <main class="mx-auto max-w-5xl px-5 py-10 sm:px-8">
    ...
    <!-- Premium members only need a compact confirmation state. -->
    <section v-else-if="isPremium">
      <h1 class="text-lg font-bold text-[#F4FFF7]">Your Premium</h1>
      <div class="mx-auto flex min-h-64 max-w-md flex-col items-center justify-center px-5 text-center">
        <Check :size="42" :stroke-width="2" class="text-[#F4FFF7]" />
        <h2 class="mt-3 text-base font-bold text-[#F4FFF7]">You're all set</h2>
        <p class="mt-2 text-sm leading-5 text-[#A1A1AA]">
          <template v-if="premiumUntil">Your Premium membership is active until {{ formatDate(premiumUntil) }}.</template>
          <template v-else>Your Premium membership is active.</template>
        </p>
      </div>
    </section>
    ...
    <template v-else>
      ...
      <section v-if="!order" class="mt-8 grid gap-5 md:grid-cols-2">
