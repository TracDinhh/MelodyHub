...
import { AtSign, Check, Eye, EyeOff, Mail, Mic2, Music2, UserRound } from '@lucide/vue';
...
const title = computed(() => mode.value === 'login' ? 'Sign in to your studio' : 'Create your MelodyHub account');
const subtitle = computed(() => mode.value === 'login'
  ? 'Manage your music, view stats, and grow your audience.'
  : 'Register, then request artist access from the Studio.'
);
...
function validateLogin() {
  clearErrors();
  if (!loginForm.usernameOrEmail.trim()) errors.usernameOrEmail = 'Enter your username or email.';
...
function validateRegistration() {
  ...
  if (username.length < 3 || username.length > 50) errors.username = 'Username must be between 3 and 50 characters.';
  if (displayName.length > 100) errors.displayName = 'Display name must be 100 characters or fewer.';
  if (!email || email.length > 255 || !emailPattern.test(email)) errors.email = 'Enter a valid email address up to 255 characters.';
  ...
  if (!registerForm.terms) errors.terms = 'You must accept the Terms of Service.';
...
function mapBackendError(error) {
  ...
  if (mode.value === 'login') {
    if (code === 'INVALID_CREDENTIALS') { errors.general = 'The username, email, or password is incorrect.'; return; }
    if (code === 'USER_BANNED') { errors.general = 'This account has been suspended.'; return; }
  }
  ...
  if (field) { errors[field] = message; return; }

  errors.general = code === 'NETWORK_ERROR'
...
async function submit() {
  ...
  try {
    if (mode.value === 'login') {
      ...
      // Studio access is membership-based (GET /api/me/artists decides what
      // happens next) — any authenticated user can open the Studio entry page.
      await router.push({ name: 'studio-entry' });
    } else {
      // Register as a regular user, then let the Studio access flow guide them.
...
<template>
  <section class="w-full max-w-md">
    <div class="mb-8 text-center">
      <div class="mx-auto mb-3 inline-flex items-center gap-2 rounded-full border border-[#16C65A]/30 bg-[#16C65A]/10 px-3 py-1 text-[10px] font-bold text-[#16C65A]">
        <Mic2 :size="12" /> FOR ARTISTS
      </div>
      <RouterLink :to="{ name: 'artist-landing' }" class="block">
        <img :src="logoUrl" alt="MelodyHub logo" class="mx-auto h-20 w-56 object-contain" />
      </RouterLink>
      <h1 class="mt-5 text-3xl font-black text-white">{{ title }}</h1>
      <p class="mt-2 text-sm text-[#858585]">{{ subtitle }}</p>
    </div>
    ...
    <div class="rounded-xl border border-white/10 bg-[#111827]/90 p-5 shadow-2xl shadow-black/40 backdrop-blur-xl sm:p-7">
      ...
      <form class="space-y-4" novalidate @submit.prevent="submit">
        ...
        <label class="melodyhub-field">
          <span>Password</span>
          <div class="field-inline">
            <Mic2 :size="15" class="shrink-0 text-[#71717A]" />
            <input
        ...
        >
          ...
          <span v-else>{{ mode === 'login' ? 'LOGIN TO ARTIST STUDIO' : 'REGISTER & REQUEST ACCESS' }}</span>
        </button>
      </form>

    </div>

    <div class="mt-6 flex flex-col items-center gap-2 text-xs">
      <RouterLink :to="{ name: 'home' }" class="text-[#71717A] transition hover:text-white">← Back to MelodyHub</RouterLink>
      <RouterLink :to="{ name: 'artist-landing' }" class="text-[#71717A] transition hover:text-white">← Back to Artist landing</RouterLink>
    </div>
  </section>
</template>
